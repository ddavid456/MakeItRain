...
def buildTemplate(gzipContent: bytes, fileName: str):
    ...
    print(f'Length of content {len(gzipContent)}')
    hexData = ", ".join(["0x%0.2X" % byt for byt in gzipContent])
    formatting = ','.join(l + '\n' * (n % 14 == 2) for n, l in enumerate(hexData.split(",")))
    ...
    buildStringContent = f'const uint8_t PAGE_{fileString}[] PROGMEM = {{{formatting}}};'
